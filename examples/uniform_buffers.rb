...
rebuild_swap_chain = proc do
  ...
  descriptor_set_layout = pipeline.add_descriptor_set_layout descriptors: [{ binding: 0,
                                                             type: :uniform_buffer,
                                                             stages: [:vertex] }]
  descriptor_sets = descriptor_set_pool.alloc(layouts: [descriptor_set_layout] * $swapchain.size)
  descriptor_sets.each_with_index { |set, i| set.update_uniform_buffer(UniformBuffers[i], binding: 0) }
